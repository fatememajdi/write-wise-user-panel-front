...
const Features: React.FC = () => {
    ...
    return <div className='bg-featuresPage-pattern bg-cover bg-no-repeat col-12 min-h-[100vh] flex flex-col' >
        ...
        <div className='col-12 flex flex-1 flex-row items-center pt-[194px] mini-tablet:pt-[142px] mac:pt-[155px] tablet:pt-[159px] pb-[74px] tablet:pb-[40px] justify-between sm:pt-[80px] sm:pr-[25px] sm:pb-[100px] sm:pl-[31px] mini-tablet:pl-[73px] mini-tablet:pb-[79px] mini-tablet:pr-[82px] '>
            <AnimatePresence>
                <div className='flex flex-col pl-[148px] tablet:pl-[68px] w-[60%] mini-tablet:w-full mini-tablet:pl-0 sm:max-w-full sm:p-0 '>
                    {
                        featuresItems.map((item, index) =>
                            <div key={index}>
                                <motion.div onClick={() => changeSelectedItem(index)}
                                    animate={selectedItem === index ?
                                        { color: '#2E4057', opacity: 1, marginLeft: isMobile || isMiniTablet ? 0 : isTablet ? 44 : isMac ? 43 : 71, fontSize: isMobile ? 16 : isTablet ? 20 : isMac ? 37 : 48, fontWeight: isMac || isTablet ? 600 : 800 }
                                        : { color: '#252525', opacity: 0.5, marginLeft: 0, fontSize: isMobile ? 16 : isMac ? 25 : 32, fontWeight: 600 }}
                                    transition={{ type: "spring", duration: 1 }}
                                    className={' flex flex-col items-center mb-[56px] tablet:mb-[39px] mini-tablet:mb-[20px] mac:mb-[42px] pr-[10px] cursor-pointer leading-[133%] w-fit pb-[8px] hover:opacity-100 sm:mt-[8px] mini-tablet:text-[20px] mini-tablet:font-semibold mini-tablet:leadig-[17px] ' + (selectedItem === index ? ' tablet:text-[20px] text-[48px] mac:text-[37px] mac:font-semibold font-extrabold sm:text-[16px] sm:font-semibold tablet:font-semibold sm:leading-[133%] ' : ' tablet:text-[20px] text-[32px] font-semibold mac:text-[25px] sm:text-[16px] ')}>
                                    <div className="flex flex-row w-full items-center ">
                                        <Image
                                            className={'mr-[16px] mac:mr-[6px] tablet:mr-[11px] mini-tablet:w-[30px] mini-tablet:h-[30px] mini-tablet:mr-[14px] ' + (selectedItem === index ? 'h-[70px] w-[70px] mac:h-[55px] mac:w-[55px] tablet:h-[40px] tablet:w-[40px] sm:h-[27px] sm:w-[27px] ' : 'h-[38px] w-[38px] tablet:h-[30px] tablet:w-[30px]  mac:h-[35px] mac:w-[35px] sm:h-[27px] sm:w-[27px] ')}
...

                                {
                                    ...
                                    <motion.div
                                        animate={{ height: selectedItem === index ? 'fit-content' : 0, opacity: selectedItem === index ? 1 : 0, marginBottom: isMiniTablet && selectedItem === index ? 37 : 0 }}
                                        transition={{ type: "spring", duration: 1 }}
                                        className='lg:hidden h-0 overflow-hidden mt-[25px] sm:flex mini-tablet:flex '>
                                        <h6 className="leading-[36px] mini-tablet:leading-[22px] ">{item.description}</h6>
                                    </motion.div>
                                }
                            </div>
                        )
                    ...
                    {
                        isMiniTablet &&
                        <button className="bg-red text-whiteText w-[96px] h-[45px] text-[15px] font-bold leading-[31px] rounded-[6px] mt-[66px] ">
                            Sign Up
                        </button>
                    }
                </div>
